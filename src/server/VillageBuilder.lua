...
-- Server-side: ReplicatedStorage is immediately available
...
local function createBuilding(buildingType: string, position: Vector3, rotation: number): Model
    ...
    -- Core buildings only: TownHall, LumberMill, Barracks, GoldMine
    local dimensions = {
    ...
    -- Roof (all buildings have roofs now)
    ...
    -- Chimney (all buildings have chimneys now)
...
local function createFarmZone()
    -- Create stable (if defined)
    if VillageLayout.FarmZone.Stable then
        local stablePos = VillageLayout.GetWorldPosition(VillageLayout.FarmZone.Stable.position)
        createStable(stablePos, _farmFolder)
    end

    -- Create wheat fields with fences (if defined)
    if VillageLayout.FarmZone.WheatField1 then
        local fieldData = VillageLayout.FarmZone.WheatField1
    ...
    if VillageLayout.FarmZone.WheatField2 then
        local fieldData = VillageLayout.FarmZone.WheatField2
        local fieldPos = VillageLayout.GetWorldPosition(fieldData.position)
        local fieldSize = fieldData.size
        createWheatField(fieldPos, fieldSize, _farmFolder)
        if fieldData.fenced then
...
local function createDecorations()
    ...
    if decorData.MarketStalls then
...
local function createBoundary()
    ...
    -- Gate (only if defined with position)
    if VillageLayout.Gate and VillageLayout.Gate.position then
        local gatePos = VillageLayout.GetWorldPosition(VillageLayout.Gate.position)
        createGate(gatePos, VillageLayout.Gate.rotation or 0, _boundaryFolder)
    end
end
