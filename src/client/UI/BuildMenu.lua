...
local function createBuildingCard(buildingDef: any, parent: GuiObject): Frame
    ...
    local iconLabel = Components.CreateLabel({
        Name = "IconText",
        Text = string.sub(buildingDef.name, 1, 2),
...
function formatCost(cost: {gold: number?, wood: number?, food: number?}): string
    local parts = {}
...
--[[
    Populates the building grid with buildings of the current category.
]]
local function populateBuildingGrid(parent: Frame)
    ...
    local buildings = BuildingData.GetByCategory(_currentCategory)

    -- Create cards
    for _, buildingDef in buildings do
        local card = createBuildingCard(buildingDef, parent)
